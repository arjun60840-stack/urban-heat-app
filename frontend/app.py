...
# Use BACKEND_URL env var when running in Docker, otherwise fall back to Render
BACKEND_URL = os.getenv("BACKEND_URL", "https://urban-heat-app.onrender.com")
# API key for authenticating with the secured backend
API_KEY = os.getenv("SECRET_API_KEY", "")
# Shared headers sent with every backend request
API_HEADERS = {"X-API-Key": API_KEY} if API_KEY else {}


@st.cache_data(ttl=600)
def get_city_analysis(city_name):
    """Fetch and cache heat-map + hotspot data for a city (cached 10 min)."""
    requests.post(
        f"{BACKEND_URL}/analysis/analyze-city",
        json={"city_name": city_name, "latitude": 40.7128, "longitude": -74.0060},
        headers=API_HEADERS
    )
    res_map = requests.get(f"{BACKEND_URL}/analysis/heat-map?city_name={city_name}", headers=API_HEADERS)
    res_hotspots = requests.get(f"{BACKEND_URL}/analysis/hotspots?city_name={city_name}", headers=API_HEADERS)
    ...
    with st.spinner(f"Analyzing heat data for {city_name}..."):
        try:
            ...
            if map_data and hotspot_data_json:
                ...
                res_recs = requests.post(
                    f"{BACKEND_URL}/interventions/recommendations",
                    json={"city_name": city_name, "risk_category": overall_risk},
                    headers=API_HEADERS
                )
            ...
            if st.button("Run Simulation"):
                sim_res = requests.post(
                    f"{BACKEND_URL}/interventions/simulate",
                    json={
                        "city_name": city_name,
            ...
            pred_res = requests.post(
                ...
                json={"city_name": city_name, "months_ahead": 12},
                headers=API_HEADERS
            )
            ...
            if st.button("Generate PDF Report"):
                rep_res = requests.post(
                    f"{BACKEND_URL}/reports/generate-report",
                    json={
                        "city_name": city_name,
                ...
                if rep_res.status_code == 200:
                    ...
                    st.markdown(f"[📥 Download PDF Report]({BACKEND_URL}{report_url})")
